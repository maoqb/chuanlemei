...
首页的拍照主按钮是唯一记录入口，衣橱顶部加号是唯一添加入口。底部 Tab 子项占满导航栏高度，图标与文字作为一个整体居中，保证视觉位置和点击热区一致。

分类环图以正方形画布的最短边为基准，并按照描边半径向内收缩绘制区域，避免粗描边在 Canvas 四边被裁切。Debug 变体在空数据库时从专属资源生成演示衣橱和近 30 天记录，Release 变体使用空实现且不打包演示图片。
